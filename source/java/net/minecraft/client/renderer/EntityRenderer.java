...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    private void addRainParticles()
    {
        ...
        if (f != 0.0F && Config.isRainSplash())
        {
            ...
            for (int k = 0; k < j; ++k)
            {
                ...
                if (blockpos1.getY() <= blockpos.getY() + b0 && blockpos1.getY() >= blockpos.getY() - b0 && biomegenbase.canRain() && biomegenbase.getFloatTemperature(blockpos1) >= 0.15F)
                {
        ...
        if (f5 > 0.0F)
        {
            ...
            for (int i1 = k - b0; i1 <= k + b0; ++i1)
            {
                for (int j1 = i - b0; j1 <= i + b0; ++j1)
                {
                    ...
                    blockpos$mutableblockpos.set(j1, 0, i1);
                    ...
                    if (biomegenbase.canRain() || biomegenbase.getEnableSnow())
                    {
                        ...
                        if (i2 != j2)
                        {
                            ...
                            blockpos$mutableblockpos.set(j1, i2, i1);
                            ...
                            if (worldclient.getWorldChunkManager().getTemperatureAtHeight(f1, l1) >= 0.15F)
                            {
                                ...
                                blockpos$mutableblockpos.set(j1, k2, i1);
    ...
    private void setupFog(int startCoords, float partialTicks)
    {
        ...
        else
        {
            ...
            {
                Reflector.callVoid(Reflector.ForgeHooksClient_onFogRender, new Object[] {this, entity, block, Float.valueOf(partialTicks), Integer.valueOf(startCoords), Float.valueOf(f)});
            }
        }
