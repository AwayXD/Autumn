...
public class Cartesian {
    ...
    private static <T> T[] createArray(Class<? super T> clazz, int size) {
        return (T[]) Array.newInstance(clazz, size);
    }

    static class GetList<T> implements Function<Object[], List<T>> {
        @Override
        public List<T> apply(Object[] input) {
            return Arrays.asList((T[]) input);
        }
    }
    ...
    static class Product<T> implements Iterable<T[]> {
        ...
        static class ProductIterator<T> extends UnmodifiableIterator<T[]> {
            ...
            private ProductIterator(Class<T> clazz, Iterable<? extends T>[] iterables) {
                this.index = -2;
                this.iterables = iterables;
                this.iterators = (Iterator<? extends T>[]) Cartesian.createArray(Iterator.class, this.iterables.length);
            ...
            public boolean hasNext() {
                if (this.index == -2) {
                    ...
                    for (Iterator<? extends T> iterator : this.iterators) {
                        if (!iterator.hasNext()) {
                            this.endOfData();
                            return false;
                        }
